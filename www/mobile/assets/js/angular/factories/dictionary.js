...
app.factory('Dictionary',['Utils','Const',
  ...
  function getByEl(element,attr,name,values,defProperty) {
    if (element) {
      if (element.hasAttribute(attr)) {
        ...
        if (v) {
          ...
          if (v) {
            return Utils.format(v,values);
          } else return getByEl(element.parentElement,attr,name,values,defProperty);
        } else return getByEl(element.parentElement,attr,name,values,defProperty);
      } else return getByEl(element.parentElement,attr,name,values,defProperty);
    } else {
  ...
  return {
    ...
    dic: function(element,defProperty) {
      return function(name,values) {
        return getByEl(element[0],'data-ng-controller',name,values,defProperty);
      }
    },
